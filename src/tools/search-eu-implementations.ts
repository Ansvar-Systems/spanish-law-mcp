/**
 * search_eu_implementations — Search EU directives/regulations referenced by Spanish legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_spanish_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as spanish_statute_count
  ...
  if (input.has_spanish_implementation) {
    sql += ' HAVING spanish_statute_count > 0';
  }
