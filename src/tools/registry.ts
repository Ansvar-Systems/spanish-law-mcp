/**
 * Tool registry for Spanish Law MCP Server.
...
import { getSpanishImplementations, type GetSpanishImplementationsInput } from './get-spanish-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Federal Register of Legislation (Spanish Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Spanish legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Spanish statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Spanish statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Spanish legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Spanish statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Spain?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Spanish legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Spanish statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Spanish statute references or aligns with. ' +
      'Spain is not an EU member but Spanish laws may reference EU directives/regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Spanish statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_spanish_implementations',
    description:
      'Find all Spanish statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Spanish statutes. ' +
      'Note: Spain does not transpose EU law but may reference or align with EU frameworks voluntarily.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Spanish legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_spanish_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Spanish legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Spanish statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Spanish statute or provision. ' +
      ...
      'Note: Spain is not bound by EU law; this checks voluntary alignment and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Spanish statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_spanish_implementations':
          result = await getSpanishImplementations(db, args as unknown as GetSpanishImplementationsInput);
          break;
