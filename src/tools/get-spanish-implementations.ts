/**
 * get_spanish_implementations — Find Spanish statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetSpanishImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface SpanishImplementationResult {
  document_id: string;
...
export async function getSpanishImplementations(
  db: InstanceType<typeof Database>,
  input: GetSpanishImplementationsInput,
): Promise<ToolResponse<SpanishImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SpanishImplementationResult[];
