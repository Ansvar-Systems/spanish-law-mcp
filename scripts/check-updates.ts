#!/usr/bin/env tsx
/**
 * Spanish Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.boe.es';
const PORTAL_NAME = 'BOE (Boletin Oficial del Estado)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/spanish-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Spanish Law MCP — Data Freshness Check');
